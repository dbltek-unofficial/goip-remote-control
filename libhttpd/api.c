...
**
**
** $Id: api.c,v 1.1.1.1 2012/03/02 02:33:53 elwin Exp $
**
*/
...
char* httpdUrlEncode(str) char* str;
{
    char* new,
        *cp;
...
httpd* httpdCreate(host, port) char* host;
int port;
{
    httpd* new;
    int sock,
    ...
    httpd* server;
{
	...
	*/
        ...
        if (inHeaders) {
            ...
            if (strncasecmp(buf, "Authorization: ", 15) == 0) {
                ...
                if (strncmp(cp, "Basic ", 6) != 0) {
                    ...
                    cp = strchr(cp, ' ') + 1;
                    len = _httpd_decode(cp, authBuf, 100);
                    authBuf[len] = '\0';
    ...
    //printf("7\n");
    fprintf(stderr, "httpdReadRequest(): %s(%d)\n", server->request.contentType, server->request.contentLength);
    /*
	...
	*/
    if (server->request.contentLength > 0) {
        ...
        printf("9\n");
        if (strcmp(server->request.contentType, "application/x-www-form-urlencoded") == 0) {
            bzero(buf, HTTP_MAX_LEN);
            _httpd_readBuf(server, buf, server->request.contentLength);
            _httpd_storeData(server, buf);
        } else {
            fprintf(stderr, "httpdReadRequest(): %d/%d\n", server->readBufRemain, server->request.contentRemain);
            server->request.contentRemain = server->request.contentLength;
            server->request.contentRemain -= server->readBufRemain;
        }
    }
    ...
    httpd* server;
{
    ...
    bzero(&server->response, sizeof(server->response));
}
...
char* msg;
{
    ...
    if (msg[strlen(msg) - 1] != '\n')
        strcat(server->response.response, "\n");
}
...
void httpdWrite(server, buf, len)
    httpd* server;
char* buf;
int len;
{
